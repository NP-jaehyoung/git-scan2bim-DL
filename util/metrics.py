...
class Metrics:
    ...
    def PA(self):
        ...
        return torch.diagonal(self.confusion_matrix)/self.confusion_matrix.sum(dim=1) # dim=1 : 열(rows)
        
    def PP(self):
        # Pixel Precision = TP/(TP+FP)
        return torch.diagonal(self.confusion_matrix)/self.confusion_matrix.sum(dim=0) # dim=0 : 행(columns)
