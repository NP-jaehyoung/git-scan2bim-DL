# PCSdataset ; Point Cloud Segmentation Dataset
...
class PCSDataset(Dataset):
    def __init__(self,
                 root_path= "data/HePIC/", #"/home/elena/Documents/Hepic", #"../../PCSproject/Nuvole_di_punti",
                 splits_path="data/HePIC/",
                 fsl=None,
                 split="train",
                 cube_edge=128, # 3차원 데이터의 크기 (n,n,n)
                 remapping=False,
                 pretrain=None,
                 augment=True):
        ...
        if self.pretrain == "s3dis": # S3DIS (Stanford 3D Indoor Scene Dataset (S3DIS))
    ...
    def pcs2s3dis(self): # S3DIS (Stanford 3D Indoor Scene Dataset (S3DIS))
        idmap = {0:0, #'unassigned',
                 1:4, # 'beam',
                 2:5, # 'column',
                 3:7, # 'door',
    ...
    def pcs2arch(self): #Architectural Cultural Heritage Dataset
        idmap = {0:0, #'unassigned',
                 1:2, # 'beam',
                 2:1, # 'column',
                 3:5, #4, # 'door',
    ...
    def __getitem__(self, item):
        #fname은 self.root_path와 self.items[item]을 결합하여 특정 파일의 전체 경로를 만듭니다.
        fname = path.join(self.root_path, self.items[item])
        #open(fname, 'r') 파일을 읽기 모드로 엽니다. self.clean_line(l) 파일의 각 줄(l)을 처리하여: xyz: 3D 좌표 (x, y, z). lab: 레이블 (예: 클래스 이름 또는 ID).
        #zip(*...) 여러 개의 (xyz, lab) 튜플을 분리하여: xyz: 3D 좌표 리스트. lab: 레이블 리스트.
        #np.array(xyz), np.array(lab) 리스트를 NumPy 배열로 변환.
        xyz, lab = zip(*[self.clean_line(l) for l in open(fname, 'r')])
        xyz, lab = np.array(xyz), np.array(lab)
        
        #self.remapping: 특정 클래스를 다른 클래스(3개로 제한)로 매핑
        if self.remapping:
        ...
        if self.pretrain == "s3dis": # S3DIS (Stanford 3D Indoor Scene Dataset (S3DIS))
            ...
            for i in range(0,len(lab)):
                lab[i] = map[lab[i]]
        #mapping su arch
        if self.pretrain == "arch": #Architectural Cultural Heritage Dataset
            map = self.pcs2arch()
            for i in range(0,len(lab)):
                lab[i] = map[lab[i]]
        ...
        ####### (default)cube_edge x cube_edge x cube_edge 큐브 안에 들어가도록 정규화시킴 ############
        ####### 예를들어 cube_edge =256일 경우 256 x 256 x 256 큐브안에 데이터가 들어가있으면 해당 Voxel = 1 그리고 라벨 데이터 입력
        xyz -= xyz.mean(axis=0)
        xyz /= np.abs(xyz).max()

        # 학습시 데이터 증강(augmentation)을 적용하는 부분
        # 원본 데이터를 인위적으로 변형시켜 다양하게 만들어, 학습 데이터의 양과 다양성을 늘리는 방법으로
        # 실제 환경에서 일반화 능력을 향상할 수 있음
        if self.augment:
        ...
        # 유효한 포인트들만 선택(filtering) 하기 위한 작업
        #print("Before filtering: xyz shape =", xyz.shape, ", lab shape =", lab.shape)
        xyz = xyz[valid,:]
        lab = lab[valid]
        #print("After filtering: xyz shape =", xyz.shape, ", lab shape =", lab.shape)
